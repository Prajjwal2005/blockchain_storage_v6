    ...
    if "assignments" in chunk_record and isinstance(chunk_record["assignments"], list):
        for a in chunk_record["assignments"]:
            ...
            if a.get("ip") and a.get("port"):
                try:
                    return fetch_chunk_bytes_from_peer(a["ip"], a["port"], chunk_hash)
                except Exception as e:
            ...
            if a.get("node_id") and DISCOVERY_URL:
                try:
                    ...
                    if r.status_code == 200:
                        peer = r.json()
                        try:
                            return fetch_chunk_bytes_from_peer(peer["ip"], peer["port"], chunk_hash)
                        except Exception as e:
    ...
    def generate():
        for rec in chunk_records:
            try:
                ...
                LOG.error("Missing chunk during download: %s", e)
                # abort generator by raising — Flask will return a 500/502
                raise
            yield chunk_bytes
